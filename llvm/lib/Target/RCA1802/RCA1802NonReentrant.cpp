//===-- RCA1802NonReentrant.cpp - RCA1802 NonReentrant Pass -----------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the RCA1802 NonReentrant pass.
//
...
// because such functions can have at rca1802t one invocation active at any given
// time. Along the way, this pass performs a norecurse analysis as well.
//
//===----------------------------------------------------------------------===//

#include "RCA1802NonReentrant.h"
...
#define DEBUG_TYPE "rca1802-nonreentrant"

using namespace llvm;

namespace {

struct RCA1802NonReentrantImpl {
  CallGraph &CG;
  ...
  RCA1802NonReentrantImpl(CallGraph &CG) : CG(CG) {
    initializeRCA1802NonReentrantPass(*PassRegistry::getPassRegistry());
  }
...
bool RCA1802NonReentrantImpl::run(Module &M) {
  LLVM_DEBUG(dbgs() << "**** RCA1802 NonReentrant Pass ****\n");
...
bool RCA1802NonReentrantImpl::runOnSCC(CallGraphSCC &SCC) {
...
void RCA1802NonReentrantImpl::markReentrant(const CallGraphNode &CGN) {
...
void RCA1802NonReentrantImpl::visitNorecurseInterrupt(const CallGraphNode &CGN) {
  if (Reentrant.contains(&CGN))
    return;
...
struct RCA1802NonReentrant : public ModulePass {
  static char ID; // Pass identification, replacement for typeid

  RCA1802NonReentrant() : ModulePass(ID) {
    initializeRCA1802NonReentrantPass(*PassRegistry::getPassRegistry());
  }
...
bool RCA1802NonReentrant::runOnModule(Module &M) {
  LLVM_DEBUG(dbgs() << "**** RCA1802 NonReentrant Pass ****\n");

  CallGraph &CG = getAnalysis<CallGraphWrapperPass>().getCallGraph();
  return RCA1802NonReentrantImpl(CG).run(M);
}

void RCA1802NonReentrant::getAnalysisUsage(AnalysisUsage &AU) const {
  AU.addRequired<CallGraphWrapperPass>();
  AU.addPreserved<CallGraphWrapperPass>();
}

PreservedAnalyses RCA1802NonReentrantPass::run(Module &M,
                                           ModuleAnalysisManager &AM) {
  LLVM_DEBUG(dbgs() << "**** RCA1802 NonReentrant Pass ****\n");

  CallGraph &CG = AM.getResult<CallGraphAnalysis>(M);
  bool Changed = RCA1802NonReentrantImpl(CG).run(M);
  return Changed ? PreservedAnalyses::none() : PreservedAnalyses::all();
}

char RCA1802NonReentrant::ID = 0;

INITIALIZE_PASS(
    RCA1802NonReentrant, DEBUG_TYPE,
    "Detect non-reentrant functions via detailed call graph analysis", false,
    false)

ModulePass *llvm::createRCA1802NonReentrantPass() { return new RCA1802NonReentrant(); }
